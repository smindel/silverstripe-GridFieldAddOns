	...
	$.entwine('ss', function($) {
		...
		$('.ss-gridfield .action.gridfield-setup').entwine({
			onclick: function() {
				...
				var dialogelement = $('#GridFieldUserColumnsDialog').length ? $('#GridFieldUserColumnsDialog') : $('<div id="GridFieldUserColumnsDialog" data-url="' + updateurl + '"></div>').appendTo('body')
		...
		$('#GridFieldUserColumnsDialog').entwine({
			save: function() {
				...
				$.post(url, data, function(a,b){ console.log([a,b]); });
			}
		});
	});
